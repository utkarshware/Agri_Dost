...
try:
    model = joblib.load('crop_price_model_xgboost_improved.pkl')
    print("Model loaded successfully")
except FileNotFoundError:
    print("Error: Model file 'crop_price_model_xgboost_improved.pkl' not found. Please ensure the model is saved in the same directory.")
    exit(1)

# Load the combined dataset to get the most recent price and crop mapping
try:
    combined_df = pd.read_excel('combined_crop_data.xlsx')
    combined_df['Dates'] = pd.to_datetime(combined_df['Dates'])
except FileNotFoundError:
    print("Error: Dataset file 'combined_crop_data.xlsx' not found. Please ensure the dataset is in the same directory.")
    exit(1)
...
print("Available crops:", list(crop_name_to_code.keys()))
...
def predict_crop_price(crop_name, prediction_date, planting_date):
    # Get the code for the crop
    crop_code = crop_name_to_code.get(crop_name)
    if crop_code is None:
        print(f"Error: {crop_name} not found in the dataset. Available crops are: {list(crop_name_to_code.keys())}")
        exit(1)

    # Get the most recent price for the crop (as a proxy for Prev_Price)
    most_recent_price = combined_df[combined_df['Crop'] == crop_name]['Price'].iloc[-1]
    print(f"\nUsing the most recent {crop_name} price as a reference: {most_recent_price}")
...
# Get user input
print("\nEnter the details to predict the price of a crop:")
...
try:
    ...
    print(f"\n===== PREDICTED PRICE =====")
    print(f"The predicted price for {crop_name} on {prediction_date.strftime('%Y-%m-%d')} is: {predicted_price:.2f}")
    print(f"==========================\n")
...
if not historical_prices.empty:
    print(f"For context, here are the historical prices for {crop_name} in {month_name} (based on past data):")
    print("These are NOT the predicted price, but they can help you understand typical prices in this month.")
    print(historical_prices.describe())
else:
    print(f"No historical data available for {crop_name} in {month_name}.")